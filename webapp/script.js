...
function init() {
	ws = new WebSocket("ws://" + window.location.host + "/TypeRacer/typeracer/" + rname.value);
	console.log("Initiated"+rname.value)

	ws.onopen = function(event) {
		console.log("WebSocket connection established on room " + rname.value);
	};
	...
	ws.onmessage = function(event) {
		...
		if (message === "WAIT") {
			...
			const [_, sessionId, progress, wpm] = message.split(":");
            updateProgressBar(sessionId, progress, wpm);
		}
...
function updateProgressBar(sessionId, progress, wpm) {
    let progressBarWrapper;
    let progressBar;
    let wpmDisplay;

    if (userProgressBars.has(sessionId)) {
        progressBarWrapper = userProgressBars.get(sessionId);
        ...
        userProgressBars.set(sessionId, progressBarWrapper);
    }
...
quoteInputElement.addEventListener('input', () => {
    ...
    const user_name = document.getElementById("user_name").value	
	ws.send(`PROGRESS:${progressPercentage}:${user_name} ${wpm}`);
